...
sebuah_data <- 20 #assign 10 to 'a'
sebuah_data = 20
20 -> sebuah_data #assign 10 to 'a'
20 = sebuah_data
...
for (i in 1:10) {
  # i-th elements of 'ul' squared into 'i' -th position
  usq[i] <- ul[i] *ul[i]
  print(usq[i])
}
...
# Fungsi apply family
X <- matrix(rnorm (30), nrow = 5, ncol = 6) #matrix 5 x 6
apply(X , 2, sum)
